...
public interface PostRepo extends JpaRepository<Post, Long> {
    ...
    @Query("select p, count(c), count(l) from Post p left join Comment c on c.post.id = p.id left join Likes l on l.post.id = p.id where p.author.username = :username  group by p.id order by p.creation desc")
